...
def main(config):
    ...
    # Solver for training (testing is implemented in jupyter notebook) .
    self = Solver(config)
    ...
    # Creating weights for quantum citcuits
    if config.quantum:
        weights_dict = {
            'main': torch.tensor(list(
                np.random.rand(config.layer * (config.qubits * 2 - 1)) * 2 * np.pi - np.pi),
                requires_grad=True),
            'gen': torch.tensor(list(
                np.random.rand(config.layer * (config.qubits * 2 - 1)) * 2 * np.pi - np.pi),
                requires_grad=True),
            'v1': torch.tensor(list(
                np.random.rand(config.layer * (config.qubits * 2)) * 2 * np.pi - np.pi),
                requires_grad=True),
            'v2': torch.tensor(list(
                np.random.rand(config.layer * (config.qubits * 2)) * 2 * np.pi - np.pi),
                requires_grad=True),
            'v3': torch.tensor(list(
                np.random.rand(config.layer * (config.qubits * 2 - 1)) * 2 * np.pi - np.pi),
                requires_grad=True),
        }

        if circuit not in weights_dict:
            raise ValueError("Unknown quantum circuit: {}. ".format(circuit) + "Available: " + " ".join(models))

        gen_weights = weights_dict[circuit]
    ...
    for i in range(start_iters, self.num_iters):
        ...
        # Addition of quantum part to the model
        if config.quantum:
        ...
        if (i + 1) % self.n_critic == 0:
            ...
            # Frechet distance between fake and real by bonds
            ...
            # Frechet distance between fake and real by atoms and bonds
            ...
            # Saving model checkpoint with the lowest FD score
            ...
            if fd_bond_atom_min > fd_bond_atom:
                ...
                with open(os.path.join(self.model_save_dir, 'lowest_indices_' + circuit + '.csv'), 'a', newline='') as file:
        ...
        if (i + 1) % self.log_step == 0:
            ...
            with open(os.path.join(self.result_dir, 'metric_' + config.circuit + '.csv'), 'a', newline='') as file:
                writer = csv.writer(file)
                writer.writerow([i + 1, et] + [torch.mean(rewardR).item(), torch.mean(rewardF).item()] +
                                [value for tag, value in loss.items()])
            with open(os.path.join(self.model_save_dir, 'quantum_weights_' + config.circuit + '.csv'), 'a', newline='') as file:
                writer = csv.writer(file)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--layer', type=int, default=2, help='number of repeated variational quantum layer')
    parser.add_argument('--qubits', type=int, default=5, help='number of qubits and dimension of domain labels')
    ...
    if circuit == 'main':
        ...
        def gen_circuit(w):
            ...
            for layer in range(config.layer):
                ...
                for i in range(config.qubits - 1):
                    qml.RZ(w[i + config.qubits], wires=i + 1)
                    qml.CNOT(wires=[i, i + 1])
                qml.RZ(w[-1], wires=config.qubits - 1)
        ...
        def gen_circuit(w):
            ...
            for layer in range(config.layer):
                ...
                qml.RZ(w[-1], wires=config.qubits - 1)
                for i in range(config.qubits - 1):
                    qml.CNOT(wires=[config.qubits - 1 - i, config.qubits - 2 - i])
    ...
    if config.quantum:
        config.log_dir = 'qgan-{}-{}-q{}-l{}/logs'.format(config.circuit, config.complexity, config.qubits,
                                                          config.layer)
        config.model_save_dir = 'qgan-{}-{}-q{}-l{}/models'.format(config.circuit, config.complexity, config.qubits,
                                                                   config.layer)
        config.sample_dir = 'qgan-{}-{}-q{}-l{}/samples'.format(config.circuit, config.complexity, config.qubits,
                                                                config.layer)
        config.result_dir = 'qgan-{}-{}-q{}-l{}/results'.format(config.circuit, config.complexity, config.qubits,
                                                                config.layer)
